...
import { getNEWS2Color, getRiskLevelColor, createProgressBarStyles } from '../../theme/utils';
import { DiagonalLines } from '../common/DiagonalLines';

interface NEWS2ScoreDisplayProps {
  score: number | null;
  riskLevel: 'low' | 'medium' | 'high' | null;
  isComplete: boolean;
}

export const NEWS2ScoreDisplay: React.FC<NEWS2ScoreDisplayProps> = ({
  score,
  riskLevel,
  isComplete,
}) => {
  ...
  const renderScoreBar = () => {
    const backgroundColor = getNEWS2Color(score);
    const maxScore = 20; // NEWS2 maximum theoretical score
    ...
    return (
      <View style={componentStyles.progressContainer}>
        ...
        <View style={componentStyles.scoreTextContainer}>
          <Text style={componentStyles.scoreLabel}>NEWS2</Text>
  ...
  const renderRiskLevel = () => {
    ...
    const riskText = riskLevel.charAt(0).toUpperCase() + riskLevel.slice(1) + ' Risk';
